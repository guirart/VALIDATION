...
assert.match(api,/importSyntheticCases\(body,principal\.userId\)/);
assert.match(api,/importSyntheticCases\(body,session\.profileId\)/);
assert.match(api,/owner_id:normalizedOwnerId/);
assert.match(api,/owner_id=eq\.\$\{encodeURIComponent\(normalizedOwnerId\)\}/);
assert.match(api,/orphaned_synthetic_case_claimed/);
