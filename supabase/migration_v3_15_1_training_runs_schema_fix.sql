-- Veredicta 3.15.1 - database schema alignment for autonomous blind training runs
-- Safe/idempotent repair for deployments where app 3.15.x was released before migration_v3_15_training_runs.sql was applied.
...
alter table public.cases add column if not exists run_id uuid;
...
begin
  if not exists (
    ...
    where conname = 'cases_run_id_fkey'
      and conrelid = 'public.cases'::regclass
  ) then
    alter table public.cases
      add constraint cases_run_id_fkey
      foreign key (run_id) references public.training_runs(id) on delete set null;
  end if;
end $$;
...
-- select column_name from information_schema.columns where table_schema='public' and table_name='cases' and column_name in ('run_id','training_round','training_order');
-- select to_regclass('public.training_runs') as training_runs, to_regclass('public.training_expected') as training_expected;
