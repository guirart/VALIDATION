...
function expectedPoints(_classification,facts){
  const out={};
  for(let i=1;i<=15;i++)out[String(i)]={legal_result:'atende'};
  ...
  // 2. Filtro geral: finalidade rural + perda mínima + nexo. Dossiê incompleto não vira reprovação automática.
  if(unknown(facts.eligibleRural)||unknown(facts.lossPercent)||unknown(facts.causalNexus))set(2,'inconclusivo');
  else if(!facts.eligibleRural||facts.lossPercent<30||!facts.causalNexus)set(2,'nao_atende');
  else set(2,facts.docsComplete?'atende':'inconclusivo');

  // 3. Modalidade excepcional. O gerador não inventa número de safras; quando a via geral fecha, a excepcional é dispensável.
  if(unknown(facts.eligibleRural)||unknown(facts.lossPercent)||unknown(facts.causalNexus))set(3,'inconclusivo');
  else if(facts.eligibleRural&&facts.lossPercent>=30&&facts.causalNexus)set(3,'nao_aplicavel');
  else if(!facts.eligibleRural||!facts.causalNexus||facts.lossPercent<40)set(3,'nao_atende');
  else set(3,'inconclusivo');

  // 4. Escopo da dívida depende da existência de instrumento/data identificável e finalidade rural.
  if(!facts.contractDate||unknown(facts.eligibleRural))set(4,'inconclusivo');
  else set(4,facts.eligibleRural?'atende':'nao_atende');

  // 5 é uma salvaguarda normativa: o enquadramento não cria obrigação automática de crédito.
  set(5,'atende');

  // 6. Processos judiciais: a MP não suspende automaticamente medidas em curso.
  if(unknown(facts.hasJudicialAction))set(6,'inconclusivo');
  else set(6,facts.hasJudicialAction?'nao_atende':'atende');

  // 7. Tempestividade/formalização.
  if(unknown(facts.formalizationTimely))set(7,'inconclusivo');
  else set(7,facts.formalizationTimely?'atende':'nao_atende');

  // 8. Laudo: ausência de nexo reprova; nexo com documentação incompleta é inconclusivo.
  if(unknown(facts.causalNexus))set(8,'inconclusivo');
  else if(!facts.causalNexus)set(8,'nao_atende');
  else set(8,facts.docsComplete?'atende':'inconclusivo');

  // 9. Independência/consistência técnica: sem documentação suficiente, não presumir fraude nem aprovação.
  set(9,facts.docsComplete?'atende':'inconclusivo');

  // 10. O gerador produz uma única operação por caso; mantém-se atendido salvo ausência total de dados.
  set(10,unknown(facts.amount)?'inconclusivo':'atende');

  // 11. Linha de excedente não é presumida pelo dossiê sintético.
  set(11,'nao_aplicavel');

  // 12. Garantias/novação exigem documentação contratual suficiente; não dependem da existência de ação judicial.
  if(!facts.guarantee)set(12,'inconclusivo');
  else set(12,facts.docsComplete?'atende':'inconclusivo');

  // 13. Valores indenizados não podem ser contados novamente.
  if(unknown(facts.indemnity))set(13,'inconclusivo');
  else set(13,facts.indemnity?'nao_atende':'atende');

  // 14. DAU é impeditivo expresso para a via do art. 1º.
  if(unknown(facts.hasDau))set(14,'inconclusivo');
  else set(14,facts.hasDau?'nao_atende':'atende');

  // 15. Janela/prazo: usar o fato de tempestividade já materializado no dossiê.
  if(unknown(facts.formalizationTimely))set(15,'inconclusivo');
  else set(15,facts.formalizationTimely?'atende':'nao_atende');
