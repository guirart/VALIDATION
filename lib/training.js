...
function expectedPoints(classification,facts){
  ...
  // O gabarito decorre de fatos controlados, sem depender de título, nome ou posição.
...
function generateFacts(rand,classification){
  ...
  const names=['Carlos Almeida','Marina Duarte','Renato Campos','Luciana Freitas','Paulo Menezes','Helena Ribeiro','André Nogueira','Fernanda Tavares','Bruno Martins','Camila Rocha'];
  ...
  const common={
    clientName:pick(rand,names),culture:pick(rand,cultures),city:pick(rand,cities),modality:pick(rand,modalities),source:pick(rand,sources),
    ...
    seedNonce:int(rand,100000,999999)
  };
...
function contractFromFacts(f,classification){
  ...
  return [
    'INSTRUMENTO SINTÉTICO DE CRÉDITO RURAL — USO EXCLUSIVO EM TESTE',
    `Mutuário: ${f.clientName}.`,
    docs,loss,eligibility,insurance,legal,formal,
    'Este dossiê é sintético. Os fatos acima devem ser avaliados exclusivamente pelas regras e fontes jurídicas carregadas pelo Veredicta.'
  ].join('\n\n');
}
...
export function generateTrainingRound({runId,seed,round}){
  ...
  const generated=classes.map((classification,index)=>{
    ...
    return {
      ...
      title:`Dossiê rural ${crypto.createHash('sha256').update(caseSeed).digest('hex').slice(0,8).toUpperCase()}`,
