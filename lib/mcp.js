...
export function createVeredictaMcpServer(){
  ...
  server.registerTool('importar_casos_sinteticos_veredicta',{
    ...
    inputSchema:{environment:z.literal('test'),batch_id:z.string().min(1).max(120),cases:z.array(z.object({external_test_id:z.string().regex(/^VEREDICTA-TEST-\\d{3,4}$/),title:z.string().min(1),client_name:z.string().optional(),contract_text:z.string().min(1)})).min(1).max(100)},
    outputSchema:{data:z.record(z.string(),z.unknown())},annotations:WRITE_ANALYSIS,_meta:toolMeta()
