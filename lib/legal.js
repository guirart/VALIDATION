...
import { evaluateObjectiveRules } from './objectiveRules.js';
...
export const legalRegistryText = fs.readFileSync(path.join(root, 'legal/REGISTRO_FONTES_JURIDICAS.md'), 'utf8');
export const cmn5330Text = fs.readFileSync(path.join(root, 'legal/CMN_5330_2026_texto_vigente.md'), 'utf8');
export const cmn5334Text = fs.readFileSync(path.join(root, 'legal/CMN_5334_2026_texto_oficial.md'), 'utf8');
...
function deriveDisplayStatus(applicability, evidenceStatus, legalResult, contractQuote) {
  ...
  if (evidenceStatus === 'nao_consta') return 'nao_consta';
  // Uma menção literal no dossiê é evidência parcial, mesmo sem prova suficiente.
  if (evidenceStatus === 'nao_comprovado') return !String(contractQuote||'').trim() || quoteIsAbsentMarker(contractQuote) ? 'nao_consta' : 'parcial';
  return 'parcial';
}
...
// Projeta a mesma regra para análises antigas, sem reescrever histórico ou auditoria.
export function projectAnalysisDisplay(analysis) {
  if (!analysis?.analyst_json?.points) return analysis;
  return {...analysis, analyst_json:{...analysis.analyst_json, points:analysis.analyst_json.points.map(p=>{
    if (canonicalEvidenceStatus(p.evidence_status)!=='nao_comprovado' || !String(p.contract_quote||'').trim() || quoteIsAbsentMarker(p.contract_quote)) return p;
    const status=deriveDisplayStatus(canonicalApplicability(p.applicability),'nao_comprovado',canonicalLegalResult(p.legal_result),p.contract_quote);
    return {...p,display_status:status,display_label:displayLabelFromStatus(status),verdict:status==='nao_consta'||status==='nao_se_aplica'?'ausente':status};
  })}};
}
...
const REGULATORY_TEXTS = {
  'CMN-5330-2026': cmn5330Text,
  'CMN-5334-2026': cmn5334Text
};
const REQUIRED_NORMATIVE_SOURCES = ['CMN-5330-2026','CMN-5334-2026'];

function verifyRegulatoryQuote(sourceId, quote) {
  const source = REGULATORY_TEXTS[sourceId];
  if (!source) return false;
  return Boolean(String(quote||'').trim()) && exactQuoteExists(source, quote);
}

function verifyNormativeSources(analysis) {
  const errors = [];
  const raw = Array.isArray(analysis?.normative_sources) ? analysis.normative_sources : [];
  const seen = new Set();
  const normalized = raw.map(item => {
    const sourceId=String(item?.source_id||'').trim();
    const applicability=canonicalApplicability(item?.applicability);
    const quote=String(item?.quote||'').trim();
    const reasoning=String(item?.reasoning||'').trim();
    if(!REGULATORY_TEXTS[sourceId]) errors.push(`fonte normativa desconhecida: ${sourceId||'(vazia)'}`);
    if(seen.has(sourceId)) errors.push(`fonte normativa duplicada: ${sourceId}`);
    seen.add(sourceId);
    if(!APPLICABILITIES.has(applicability)) errors.push(`applicability inválido na fonte normativa ${sourceId}`);
    if(!reasoning) errors.push(`raciocínio ausente na fonte normativa ${sourceId}`);
    const quoteVerified=quote?verifyRegulatoryQuote(sourceId,quote):false;
    if(applicability!=='nao_aplicavel' && !quote) errors.push(`citação regulatória ausente em ${sourceId}`);
    if(quote && !quoteVerified) errors.push(`citação regulatória não verificada em ${sourceId}`);
    return {...item,source_id:sourceId,applicability,quote,reasoning,quote_verified:quoteVerified};
  });
  for(const sourceId of REQUIRED_NORMATIVE_SOURCES){
    if(!seen.has(sourceId)) errors.push(`fonte normativa obrigatória ausente: ${sourceId}`);
  }
  const cmn5330=normalized.find(x=>x.source_id==='CMN-5330-2026');
  if(cmn5330?.applicability==='nao_aplicavel') errors.push('Resolução CMN 5.330/2026 não pode ser marcada como não aplicável no pipeline da MP 1.376/2026');
  return {errors,normalized};
}

function verifyPointRegulatoryCitations(pointNumber,p){
  const errors=[];
  const raw=Array.isArray(p?.regulatory_citations)?p.regulatory_citations:[];
  const normalized=raw.map(c=>{
    const sourceId=String(c?.source_id||'').trim();
    const quote=String(c?.quote||'').trim();
    const legalReference=String(c?.legal_reference||'').trim();
    const reasoning=String(c?.reasoning||'').trim();
    if(!REGULATORY_TEXTS[sourceId]) errors.push(`ponto ${pointNumber}: fonte regulatória desconhecida ${sourceId||'(vazia)'}`);
    if(!legalReference) errors.push(`ponto ${pointNumber}: referência regulatória ausente em ${sourceId}`);
    if(!reasoning) errors.push(`ponto ${pointNumber}: raciocínio regulatório ausente em ${sourceId}`);
    const verified=verifyRegulatoryQuote(sourceId,quote);
    if(!quote) errors.push(`ponto ${pointNumber}: citação regulatória ausente em ${sourceId}`);
    else if(!verified) errors.push(`ponto ${pointNumber}: citação regulatória não verificada em ${sourceId}`);
    return {...c,source_id:sourceId,quote,legal_reference:legalReference,reasoning,quote_verified:verified};
  });
  const ref=String(p?.legal_reference||'');
  if(/5[.]?330\/2026|5[.]?330/i.test(ref) && !normalized.some(x=>x.source_id==='CMN-5330-2026')){
    errors.push(`ponto ${pointNumber}: referência à Resolução CMN 5.330 sem citação regulatória verificável`);
  }
  if(/5[.]?334\/2026|5[.]?334/i.test(ref) && !normalized.some(x=>x.source_id==='CMN-5334-2026')){
    errors.push(`ponto ${pointNumber}: referência à Resolução CMN 5.334 sem citação regulatória verificável`);
  }
  return {errors,normalized};
}
...
export function verifyAnalysis(analysis, contractText) {
  ...
  const normativeCheck=verifyNormativeSources(analysis);
  errors.push(...normativeCheck.errors);

  // A IA fornece somente fatos estruturados extraídos do documento.
  // Cálculos/requisitos objetivos são derivados aqui e prevalecem sobre opiniões da IA.
  const objectiveEvaluation=evaluateObjectiveRules(analysis.objective_facts||{});
  const suppliedObjective=analysis.objective_evaluation;
  if(suppliedObjective && JSON.stringify(suppliedObjective)!==JSON.stringify(objectiveEvaluation)){
    errors.push('objective_evaluation não pode ser definido pela IA; o backend recalcula os requisitos objetivos');
  }
  ...
  const points = (analysis.points || []).map((raw, idx) => {
    ...
    const displayStatus = deriveDisplayStatus(applicability, evidenceStatus, legalResult, p.contract_quote);
    ...
    const regulatoryCheck=verifyPointRegulatoryCitations(number,p);
    errors.push(...regulatoryCheck.errors);
    ...
    return {
      ...
      regulatory_citations: regulatoryCheck.normalized
    };
  });
  ...
  return {
    ...
    analysis: { ...analysis, normative_sources:normativeCheck.normalized, objective_evaluation:objectiveEvaluation, points },
    validation_debug: points.map(p => p.mp_validation_debug),
    regulatory_validation_debug:{normative_sources:normativeCheck.normalized,points:points.map(p=>({point:p.point,regulatory_citations:p.regulatory_citations}))},
    objective_validation_debug:objectiveEvaluation
  };
}
