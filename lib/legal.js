...
export function verifyAnalysis(analysis, contractText) {
  ...
  const points = (analysis.points || []).map((raw, idx) => {
    ...
    // O legado não governa mais a semântica, mas não pode contradizer a exibição derivada.
    const expectedLegacyVerdict = displayStatus === 'nao_se_aplica'
      ? 'ausente'
      : displayStatus === 'nao_consta'
        ? 'ausente'
        : displayStatus;
    if (p.verdict && canonicalVerdict(p.verdict) !== expectedLegacyVerdict) {
      errors.push(`ponto ${number}: verdict legado "${canonicalVerdict(p.verdict)}" contradiz display_status derivado "${displayStatus}"`);
    }
    ...
    return {
      ...
      verdict: displayStatus === 'nao_se_aplica' || displayStatus === 'nao_consta'
        ? 'ausente'
        : displayStatus,
