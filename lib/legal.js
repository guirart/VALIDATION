...
function quoteIsAbsentMarker(s) {
  return /^(?:não consta|não comprovado|ausente no documento|não localizado|não informado|não se aplica)(?:\b|[.:;-])/i.test(String(s || '').trim());
}
...
// Ponto 6 é, por desenho do checklist, uma conclusão fundada no silêncio normativo.
// Ele não deve fabricar uma "citação literal" inexistente.
const SILENCE_POINTS = new Set([6]);
