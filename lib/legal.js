...
export function verifyAnalysis(analysis, contractText) {
  ...
  const points = (analysis.points || []).map((raw, idx) => {
    ...
    if (SILENCE_POINTS.has(number)) {
      /*
       * Pontos 6 e 12 aceitam DUAS formas válidas:
       * 1) citação literal existente na MP; OU
       * 2) silêncio normativo explicitamente assumido.
       *
       * A exceção nunca transforma paráfrase em citação.
       */
      const quote = String(p.mp_quote || '').trim();
      const literalOk = exactQuoteExists(mpText, quote);

      if (literalOk) {
        mpOk = true;
        mpVerificationMode = 'literal';
      } else {
        ...
        if (!mpOk) {
          errors.push(
            `ponto ${number}: use citação literal verificável da MP ou, se realmente houver silêncio normativo, declare-o expressamente na referência e não invente citação`
          );
        }
      }
    } else {
