...
function silenceMarker(s) {
  return /sem previsão expressa|não há previsão expressa|silêncio da mp|silêncio normativo|ausência de previsão expressa|não existe previsão expressa|não consta previsão expressa/i.test(String(s || ''));
}
...
function reasoningDeclaresSilence(s) {
  return /silêncio normativo|ausência de previsão expressa|sem previsão expressa|não há previsão expressa|não consta previsão expressa/i.test(String(s || ''));
}
...
function validateMpEvidenceForPoint(number, p) {
  ...
  const silenceAllowed = number === 6 || number === 12;
  ...
  // Citação literal válida sempre prevalece, inclusive nos pontos 6 e 12.
  if (literalOk) {
    return {
      ok:true,
      mode:'literal',
      ...
      debug:{
        ...
        silence_allowed:silenceAllowed,
  ...
  // Silêncio normativo: exceção estrita apenas nos pontos 6 e 12.
  if (silenceAllowed && refSilence && reasoningSilence && quoteSilence) {
    return {
      ok:true,
      mode:'silence',
      error:null,
  ...
  const error = silenceAllowed
    ? `ponto ${number}: informe citação literal verificável da MP ou, se o fundamento for realmente silêncio normativo, declare a ausência de previsão expressa na referência e no raciocínio sem inventar citação`
    : `citação da MP não verificada no ponto ${number}`;
...
export function verifyAnalysis(analysis, contractText) {
  ...
  return {
    ...
    validation_debug: points
      .filter(p => p.point === 6 || p.point === 12)
      .map(p => p.mp_validation_debug)
  };
}
