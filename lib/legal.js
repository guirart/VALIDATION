...
function validateMpEvidenceForPoint(number, p) {
  ...
  // Regra 1: se houver citação literal válida, ela sempre prevalece.
  if (quote && exactQuoteExists(mpText, quote)) {
    return { ok:true, mode:'literal', error:null };
  }

  // Regra 2: silêncio normativo é exceção estrita e apenas para os pontos 6 e 12.
  if (number === 6 || number === 12) {
    ...
    if (refSilence && reasoningSilence && quoteSilence) {
      return { ok:true, mode:'silence', error:null };
    }
  ...
  // Regra 3: nos demais pontos, silêncio não substitui citação.
  return {
    ok:false,
    mode:'invalid',
