...
export async function db(path, options = {}) {
  ...
  const timeoutMs = Number(process.env.SUPABASE_TIMEOUT_MS || 7000);
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      const e = new Error(`Supabase não respondeu em ${timeoutMs} ms`);
      e.code = 'SUPABASE_TIMEOUT';
      throw e;
    }
    const e = new Error(`Falha de conexão com o Supabase: ${error?.message || String(error)}`);
