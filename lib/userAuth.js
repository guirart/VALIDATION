...
export async function signUpUser({email,password,name,redirectBase=''}){
  const base=String(redirectBase||process.env.APP_BASE_URL||'').replace(/\/$/,'');
  const body={email,password,data:{name}};
  const path=base?`signup?redirect_to=${encodeURIComponent(`${base}/?email_confirmed=1`)}`:'signup';
...
export async function recoverPassword(email,{redirectBase=''}={}){
  const base=String(redirectBase||process.env.APP_BASE_URL||'').replace(/\/$/,'');
  const body={email};
  if(base) body.redirect_to=`${base}/?recovery=1`;
