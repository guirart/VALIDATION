...
async function principalFromUserId(userId, res, extra={}){
  ...
  const u = users[0];
  const subscriptionStatus=String(u.subscription_status||'').toLowerCase();
  const role=String(u.role||'user').toLowerCase();
  const subscriptionAllowed=['active','trialing'].includes(subscriptionStatus)
    || (role==='admin' && subscriptionStatus==='admin_exempt');
  if(!subscriptionAllowed){
    json(res, 402, {
      error:'Assinatura Veredicta sem adimplência ativa',
      subscription_status:u.subscription_status||'pending'
    });
    return null;
  }
  return {
...
export async function requireActionAuth(req, res) {
  ...
  // Segurança comercial: não existe bypass global para análise.
  // Toda chamada deve resolver um usuário Veredicta real e validar assinatura ativa/trialing; administradores podem usar o status explícito admin_exempt.
