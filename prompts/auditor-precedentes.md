...
## Auditoria determinística das resoluções CMN

Confira também `normative_sources` e todas as `regulatory_citations`. A Resolução CMN 5.330/2026 deve estar expressamente considerada em toda análise. A Resolução CMN 5.334/2026 deve estar expressamente considerada mesmo quando concluída como não aplicável ao caso concreto. Citação regulatória deve existir literalmente no texto oficial versionado. Se faltar fonte obrigatória, se a citação não existir ou se a análise usar redação superada, marque o finding pertinente como `divergente` ou `não encontrado` e não recomende `liberar`.
